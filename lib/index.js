...
module.exports = function(schema, options, version = 5) {
  ...
  if (!options.onlyOnDemandIndexing) {
    ...
    schema.post('deleteOne', { document: true, query: false }, postRemove);
    schema.post('remove', postRemove); // Kept for retro compliance with mongoose < 8 (or > 5 ?)
      ...
      (err, exists) => {
        ...
        if (version === 7) {
          ...
          createIndexOpts.body.settings.number_of_shards =
            settings.settings.number_of_shards || 5;
        }
